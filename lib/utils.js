...
var utils = module.exports = {
  ...
  isAuthId: function (id) {
    return typeof id == 'string' && /^(?:[a-z]{2}_)?[A-z0-9]{32}$/.test(id);
  },

  isAuthToken: function (token) {
    return this.isAuthId(token);
  },

  isOptionsHash: function (o) {
    return _.isPlainObject(o) && _.some(['api_id', 'api_token', 'idempotency_key'], function(key) {
  ...
  getOptionsFromArgs: function(args) {
    ...
    if (args.length > 0) {
      ...
      if (utils.isAuthId(arg)) {
        opts.headers['X-API-ID'] = args.pop();
        opts.headers['X-API-TOKEN'] = args.pop();
      } else if (utils.isOptionsHash(arg)) {
        var params = args.pop();
        if (params.api_id) {
          opts.headers['X-API-ID'] = params.api_id;
        }
        if (params.api_token) {
          opts.headers['X-API-TOKEN'] = params.api_token;
        }
