...
describe('utils', function() {
  ...
  describe('stringifyRequestData', function() {
    ...
    describe('Screenbeacon-specific cases', function() {
  ...
  describe('getDataFromArgs', function() {
    ...
    it('ignores an options hash', function() {
      var args = [{api_id: 'foo', api_token: 'bar'}];
      expect(utils.getDataFromArgs(args)).to.deep.equal({});
      expect(args.length).to.equal(1);
    });
    it('finds the data', function() {
      var args = [{foo: 'bar'}, {api_id: 'foo', api_token: 'bar'}];
  ...
  describe('getOptsFromArgs', function() {
    ...
    it('parses an api id', function() {
      var args = ['dlajfldjalfjdslfjks'];
      expect(utils.getOptionsFromArgs(args)).to.deep.equal({
        auth: 'dlajfldjalfjdslfjks',
        headers: {},
      });
    ...
    it('parse an idempotency key and api id (with data)', function() {
      var args = [{foo: 'bar'}, {
        api_key: 'dlajfldjalfjdslfjks',
        idempotency_key: 'foo'
      }];
      expect(utils.getOptionsFromArgs(args)).to.deep.equal({
        auth: 'dlajfldjalfjdslfjks',
    ...
    it('parse an idempotency key and api id', function() {
      var args = [{
        api_key: 'dlajfldjalfjdslfjks',
        idempotency_key: 'foo'
      }];
